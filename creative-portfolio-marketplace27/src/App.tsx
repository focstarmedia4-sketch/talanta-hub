...
import { ShieldAlert, Info, Sparkles, CheckCircle, Database, Wrench, Settings, Trash2, X, AlertCircle } from 'lucide-react';
import { supabase } from './supabaseClient';
import { 
  loadFreelancerProfilesFromSupabase,
  loadSingleProfileFromSupabase, 
...
function deduplicateProfiles(list: FreelancerProfile[]): FreelancerProfile[] {
...
function getStockImage(category: string): string {
  const images: Record<string, string> = {
    ...
    framingmoulds: 'https://images.unsplash.com/photo-1579783900882-c0d3dad7b119?auto=format&fit=crop&w=600&q=70',
    filmdirectors: 'https://images.unsplash.com/photo-1485846234645-a62644f84728?auto=format&fit=crop&w=600&q=70',
    dops: 'https://images.unsplash.com/photo-1585829365295-ab7cd400c167?auto=format&fit=crop&w=600&q=70',
    lightingdesigners: 'https://images.unsplash.com/photo-1517457373958-b7bdd4587205?auto=format&fit=crop&w=600&q=70',
    craneoperators: 'https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?auto=format&fit=crop&w=600&q=70',
    livestream: 'https://images.unsplash.com/photo-1594909122845-11baa439b7bf?auto=format&fit=crop&w=600&q=70',
  };
...
const getInitialUrlState = () => {
  if (typeof window === 'undefined') return { username: null, itemId: null, jobId: null };
  ...
  let username = searchParams.get('profile') || searchParams.get('creative') || searchParams.get('username')
              || hashParams.get('profile') || hashParams.get('creative') || hashParams.get('username') || null;

  if (!username && path.startsWith('/profile/')) {
    username = path.substring('/profile/'.length).trim().split('/')[0].split('?')[0] || null;
  }

  const itemId = searchParams.get('item') || searchParams.get('itemId')
              || hashParams.get('item') || hashParams.get('itemId') || null;

  const jobId = searchParams.get('job') || searchParams.get('jobId')
             || hashParams.get('job') || hashParams.get('jobId') || null;

  return { username, itemId, jobId };
};

export default function App() {
  // Navigation tabs
  const [currentTab, setCurrentTab] = useState<'home' | 'browse' | 'jobs' | 'inbox' | 'dashboard' | 'terms'>('home');
  ...
  const [sharedUsername, setSharedUsername] = useState<string | null>(() => getInitialUrlState().username);

  // Deep-linking shared catalogue item detector
  const [sharedItemId, setSharedItemId] = useState<string | null>(() => getInitialUrlState().itemId);

  // Deep-linking job detail view selection
  const [selectedJobId, setSelectedJobId] = useState<string | null>(() => getInitialUrlState().jobId);

  // Supabase initial loading state to prevent flash of home route when deep linking
  ...
  const [freelancers, setFreelancers] = useState<FreelancerProfile[]>(() => {
    ...
    let parsed: FreelancerProfile[] = saved ? JSON.parse(saved) : [];
    // Sanitize by filtering out legacy mock profiles and dummy admin accounts
    parsed = parsed.filter(f => 
      !f.id.startsWith('f') && 
      f.id !== 'dummy_admin_account' && 
      f.id !== 'dummy_creative_account' &&
      f.email?.toLowerCase() !== 'admin@talantahub.co.ke'
    ).map(f => ({
      ...f,
  ...
  const [pendingTab, setPendingTab] = useState<'home' | 'browse' | 'jobs' | 'inbox' | 'dashboard' | 'terms' | null>(null);
  const [showUnsavedModal, setShowUnsavedModal] = useState(false);

  const activeFreelancer = freelancers.find(f => f.id === activeRole);
  const hasUnsavedDashboardChanges = currentTab === 'dashboard' && activeFreelancer?.hasUnpublishedChanges === true;

  const handleTabChange = (targetTab: 'home' | 'browse' | 'jobs' | 'inbox' | 'dashboard' | 'terms') => {
    if (targetTab === 'home') {
      setSelectedJobId(null);
      setSharedUsername(null);
      setSharedItemId(null);
    }
    if (currentTab === 'dashboard' && targetTab !== 'dashboard' && hasUnsavedDashboardChanges) {
  ...
  const handleSaveAndNavigate = () => {
    if (activeFreelancer) {
      const cleanCopy = { ...activeFreelancer };
      delete cleanCopy.publishedVersion;
      handleUpdateProfile({
        ...activeFreelancer,
        hasUnpublishedChanges: false,
        publishedVersion: cleanCopy
      });
      triggerToast("Your dashboard changes were saved and published!");
    }
    if (pendingTab) {
  ...
  const handleDiscardAndNavigate = () => {
    if (activeFreelancer) {
      if (activeFreelancer.publishedVersion) {
        handleUpdateProfile({
          ...activeFreelancer.publishedVersion,
          hasUnpublishedChanges: false
        });
      } else {
        handleUpdateProfile({
      ...
      triggerToast("Draft changes discarded.", "info");
    }
    if (pendingTab) {
      setCurrentTab(pendingTab);
      setPendingTab(null);
  ...
  // Parse location hash on initial load & hashchange
  useEffect(() => {
    const parseUrlHash = () => {
      const hash = window.location.hash || '';
      const search = window.location.search || '';
      ...
      const jobId = hashParams.get('job') || hashParams.get('jobId') || searchParams.get('job') || searchParams.get('jobId');
      const creative = hashParams.get('creative') || hashParams.get('username') || searchParams.get('creative') || searchParams.get('username') || searchParams.get('profile');
      const item = hashParams.get('item') || hashParams.get('itemId') || searchParams.get('item') || searchParams.get('itemId');
      ...
      if (path.startsWith('/profile/')) {
        const usernameFromPath = path.substring('/profile/'.length).trim().split('/')[0].split('?')[0];
        if (usernameFromPath) {
          setSharedUsername(usernameFromPath);
          if (item) setSharedItemId(item);
          return;
        }
      }

      if (creative) {
        setSharedUsername(creative);
        if (item) setSharedItemId(item);
        setCurrentTab('browse');
        return;
      }

      const cleanTab = hash.replace(/^#\/?/, '').split('&')[0].split('=')[0];
      if (['browse', 'jobs', 'inbox', 'dashboard', 'terms'].includes(cleanTab)) {
        setCurrentTab(cleanTab as any);
      } else if (cleanTab === 'home' || cleanTab === '') {
  ...
  // Synchronize browser address bar with current active tab & sub-selections
  useEffect(() => {
    if (sharedUsername || window.location.pathname.startsWith('/profile/')) {
      return;
    }
    if (currentTab === 'home') {
      ...
      const newHash = selectedJobId ? `#job=${selectedJobId}` : '#jobs';
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `${window.location.pathname}${newHash}`);
      }
    } else if (currentTab === 'browse') {
      const newHash = '#browse';
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `${window.location.pathname}${newHash}`);
      }
    } else {
      const newHash = `#${currentTab}`;
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `${window.location.pathname}${newHash}`);
      }
    }
  }, [currentTab, selectedJobId, sharedUsername]);
  ...
  useEffect(() => {
    ...
    async function ensureProfileExists(user: any) {
      ...
      const newProfile: FreelancerProfile = {
        ...
        coverUrl: '',
        bio: 'Welcome to my Talanta Hub profile—explore my work and creative journey.',
        location: '',
        hourlyRate: 4000,
      ...
      try {
        ...
        try {
          ...
          if (dbFreelancers !== null) {
            const deduped = deduplicateProfiles(dbFreelancers);
            setFreelancers(deduped);
            activeFreelancers = deduped;
          }
        } catch (err) {
        ...
        try {
          ...
          await syncSessionState(session, 'GET_SESSION_MOUNT');
        } catch (err) {
    ...
    const syncSessionState = async (session: any, eventType: string) => {
      console.log(`[Auth App] Syncing auth session triggered by '${eventType}'`, session?.user ? `User: ${session.user.id} (${session.user.email})` : 'No session');

      if (!session || !session.user) {
        console.log('[Auth App] No active session. Resetting auth state.');
        setIsLoggedIn(false);
      ...
      const fullName = user.user_metadata?.full_name || user.user_metadata?.name || user.user_metadata?.custom_claims?.global_name;
      const avatarUrl = user.user_metadata?.avatar_url || user.user_metadata?.picture;
      const provider = user.app_metadata?.provider || 'email';

      // 1. Unified post-authentication: Retrieve existing profile or initialize Creative Dashboard
      const { profile, wasCreated } = await ensureUserProfile(
        {
          userId,
          email: userEmail,
          fullName,
      ...
      // Update freelancers state and activeIn-Memory list
      setFreelancers(prev => deduplicateProfiles([profile, ...prev]));
      activeFreelancers = deduplicateProfiles([profile, ...activeFreelancers]);

      // 2. Set authenticated state
      setIsLoggedIn(true);
      localStorage.setItem('vivid_is_logged_in', 'true');
      setActiveRole(profile.id);
      localStorage.setItem('vivid_active_role', profile.id);

      // 3. Drive redirection to dashboard on explicit sign-in or OAuth redirect
      const hasOAuthHash = window.location.hash.includes('access_token') || window.location.hash.includes('type=recovery') || window.location.search.includes('code=');
      
      if (eventType === 'SIGNED_IN' || hasOAuthHash) {
        console.log(`[Auth App] Directing authenticated user ${profile.id} (${profile.fullName}) to dashboard tab. Was Created? ${wasCreated}`);
        setCurrentTab('dashboard');

        if (hasOAuthHash) {
          console.log('[Auth App] Cleaning URL hash/search OAuth parameters');
    ...
    try {
      const res = supabase.auth.onAuthStateChange(async (event, session) => {
        console.log(`[Auth Listener] Auth state change event received: ${event}`);
        await syncSessionState(session, event);
      });
      if (res && res.data) {
  ...
  useEffect(() => {
    ...
    const itemParam = params.get('item') || params.get('itemId') || hashParams.get('item') || hashParams.get('itemId');

    const profileUser = params.get('profile') || params.get('creative') || params.get('username');
    if (profileUser) {
      const found = freelancers.find(f => f.username === profileUser || f.id === profileUser);
      setSharedUsername(found ? found.username : profileUser);
      if (itemParam) setSharedItemId(itemParam);
      return;
    }
    ...
    if (path.startsWith('/profile/')) {
      const usernameFromPath = path.substring('/profile/'.length).trim().split('/')[0].split('?')[0];
      if (usernameFromPath) {
        const found = freelancers.find(f => f.username === usernameFromPath || f.id === usernameFromPath);
        setSharedUsername(found ? found.username : usernameFromPath);
        if (itemParam) setSharedItemId(itemParam);
      }
    }
  }, [freelancers]);
  ...
  useEffect(() => {
    if (sharedUsername) {
      ...
      const urlItemId = searchParams.get('item') || searchParams.get('itemId') || sharedItemId;
      const targetUrl = urlItemId
        ? `/profile/${sharedUsername}?item=${urlItemId}`
        : `/profile/${sharedUsername}`;
      ...
      const selected = freelancers.find(f => f.username === sharedUsername || f.id === sharedUsername);
      if (selected) {
  ...
  const handleUpdateProfile = async (updated: FreelancerProfile) => {
    // Helper to calculate final profile structure
    const computeFinalProfile = (targetProfile: FreelancerProfile, targetBase: FreelancerProfile) => {
      let finalProf = targetProfile;
      const isEditingSelf = activeRole === targetBase.id;

      if (isEditingSelf) {
        if (targetProfile.hasUnpublishedChanges === false) {
          finalProf = targetProfile;
        } else {
          let publishedCopy = targetBase.publishedVersion;
          if (!publishedCopy) {
            const cleanF = { ...targetBase };
            delete cleanF.publishedVersion;
            publishedCopy = cleanF;
          } else if (targetProfile.portfolio) {
            publishedCopy = {
              ...publishedCopy,
        ...
        const mergedMain = {
          ...targetBase,
          requestedCalls: targetProfile.requestedCalls !== undefined ? targetProfile.requestedCalls : targetBase.requestedCalls,
          reviews: targetProfile.reviews !== undefined ? targetProfile.reviews : targetBase.reviews,
          analytics: targetProfile.analytics !== undefined ? targetProfile.analytics : targetBase.analytics,
          walletBalanceKsh: targetProfile.walletBalanceKsh !== undefined ? targetProfile.walletBalanceKsh : targetBase.walletBalanceKsh,
          unlockedJobIds: targetProfile.unlockedJobIds !== undefined ? targetProfile.unlockedJobIds : targetBase.unlockedJobIds,
        };

        if (targetBase.publishedVersion) {
          mergedMain.publishedVersion = {
            ...targetBase.publishedVersion,
            requestedCalls: targetProfile.requestedCalls !== undefined ? targetProfile.requestedCalls : targetBase.publishedVersion.requestedCalls,
            reviews: targetProfile.reviews !== undefined ? targetProfile.reviews : targetBase.publishedVersion.reviews,
            analytics: targetProfile.analytics !== undefined ? targetProfile.analytics : targetBase.publishedVersion.analytics,
            walletBalanceKsh: targetProfile.walletBalanceKsh !== undefined ? targetProfile.walletBalanceKsh : targetBase.publishedVersion.walletBalanceKsh,
            unlockedJobIds: targetProfile.unlockedJobIds !== undefined ? targetProfile.unlockedJobIds : targetBase.publishedVersion.unlockedJobIds,
          };
        }

        finalProf = mergedMain;
      }
      return finalProf;
    };

    // 1. Instantly update local React state synchronously so deletion/updates render immediately
    setFreelancers(prev => prev.map(f => {
      if (f.id === updated.id) {
        return computeFinalProfile(updated, f);
      }
      return f;
    }));
    ...
    try {
      // 2. Process and upload any new base64 data URLs to Supabase Storage
      uploadedProfile = await processProfileUploads(updated);

      // 3. Resolve storage paths to signed URLs for local UI rendering
      resolvedProfile = await resolveProfileUrls(uploadedProfile);

      // 4. Track and delete old, replaced files from Storage
    ...
    setFreelancers(prev => prev.map(f => {
      if (f.id === updated.id) {
        finalProfile = computeFinalProfile(resolvedProfile, f);
        return finalProfile;
      }
      return f;
    }));
  ...
  const getPublicFreelancersList = (list: FreelancerProfile[]) => {
    return list
      .filter(f => !f.isClientOnly && f.isPublic !== false && f.fullName && f.username)
      .map(f => {
        if (activeRole === f.id) {
  ...
  // Callback to delete a job brief
  const handleDeleteJob = async (jobId: string) => {
    setJobs(prev => prev.filter(j => j.id !== jobId));
    triggerToast("Job brief removed.", "info");
    try {
  ...
  const handleJoinAsCreative = async (newCreative: {
    ...
    const { profile, wasCreated } = await ensureUserProfile(
      {
        ...
        authProvider: 'email',
        extraMeta: {
          categories: newCreative.categories,
    ...
    localStorage.setItem('vivid_active_role', profile.id);
    setCurrentTab('dashboard');
    triggerToast(wasCreated ? `Welcome to Talanta Hub, ${profile.fullName}!` : `Welcome back, ${profile.fullName}!`);
  };
    ...
    setFreelancers(prevFreelancers => {
      ...
      return prevFreelancers.map(f => {
        if (f.id === freelancerId) {
          const currentAnalytics = f.analytics || {
            totalViews: 0,
            totalInquiries: 0,
            conversionRate: 0,
            viewsHistory: [],
          ...
          return {
            ...f,
            unreadMessagesCount: (f.unreadMessagesCount || 0) + 1,
            analytics: {
              ...currentAnalytics,
              totalInquiries: (currentAnalytics.totalInquiries || 0) + 1
            }
          };
        }
        return f;
  ...
  const selectedDeepProfile = freelancers.find(f => f.username === sharedUsername || f.id === sharedUsername);
  ...
  if (sharedUsername) {
      ...
      return (
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs font-bold text-slate-300 tracking-wide uppercase">Loading creative profile...</p>
          </div>
        </div>
      );
    }
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-md w-full text-center space-y-6 shadow-2xl">
            ...
            <div className="space-y-2">
              <h1 className="text-xl font-extrabold text-white tracking-tight">Creative Not Found</h1>
              <p className="text-sm text-slate-400">
                The requested creative profile could not be found.
              </p>
            </div>
            <button
              onClick={() => {
    ...
    const baseProfile = isOwner ? selectedDeepProfile : (selectedDeepProfile.publishedVersion || selectedDeepProfile);
    const displayedProfile = {
      ...baseProfile,
      portfolio: Array.isArray(selectedDeepProfile.portfolio)
        ? selectedDeepProfile.portfolio 
    ...
    return (
      <PortfolioView
        ...
        initialItemId={sharedItemId}
        onClearSharedItem={() => setSharedItemId(null)}
    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col justify-between">
      ...
      <Navbar
        ...
        onChangeRole={(newRole) => {
          if (currentTab === 'dashboard' && hasUnsavedDashboardChanges) {
            setShowUnsavedModal(true);
          } else {
      ...
      <main className={`flex-1 w-full mx-auto ${currentTab === 'home' ? 'max-w-none p-0' : 'max-w-7xl px-4 sm:px-6 lg:px-8 py-8'}`}>
        ...
        <AnimatePresence mode="wait">
          ...
          {currentTab === 'browse' && (
            ...
            >
              {sharedUsername && (() => {
                const targetProfile = freelancers.find(f => f.username === sharedUsername || f.id === sharedUsername);
                if (!targetProfile) return null;
                const isOwner = activeRole === targetProfile.id;
                const baseProf = isOwner ? targetProfile : (targetProfile.publishedVersion || targetProfile);
                const displayedProf = {
                  ...baseProf,
                  portfolio: Array.isArray(targetProfile.portfolio)
                    ? targetProfile.portfolio 
                    : (baseProf.portfolio || [])
                };
                return (
                  <PortfolioView
                    profile={displayedProf}
                    initialItemId={sharedItemId}
                    onClearSharedItem={() => setSharedItemId(null)}
                    activeRole={activeRole}
                    onUpdateProfile={handleUpdateProfile}
                    onAddReview={handleAddReview}
                    onSendMessageFromContact={(freelancerId, text, name) => {
                      handleSendMessage(freelancerId, text, name);
                    ...
                    allFreelancers={freelancers}
                    onSelectFreelancer={(id) => {
                      const sel = freelancers.find(f => f.id === id);
                      if (sel) {
                        setSharedUsername(sel.username);
                        setSharedItemId(null);
                      }
                    }}
                    isLoggedIn={isLoggedIn}
          ...
          {/* DASHBOARD PANEL (CLIENT & CREATIVE) */}
          {currentTab === 'dashboard' && (
            ...
            >
              {(() => {
                const activeFreelancer = freelancers.find(f => f.id === activeRole);
                const userEmail = activeFreelancer?.email || '';

                const effectiveProfile: FreelancerProfile = activeFreelancer ? {
                  ...activeFreelancer,
                  subscribedCategories: activeFreelancer.subscribedCategories || [],
                  portfolio: activeFreelancer.portfolio || [],
                  categorySections: activeFreelancer.categorySections || [],
                  unlockedJobIds: activeFreelancer.unlockedJobIds || [],
                  skills: activeFreelancer.skills || [],
                  notableClients: activeFreelancer.notableClients || [],
                  promotionalOffers: activeFreelancer.promotionalOffers || [],
                  offers: activeFreelancer.offers || [],
                  requestedCalls: activeFreelancer.requestedCalls || [],
                  layoutOrder: activeFreelancer.layoutOrder || ['bio', 'stats', 'skills', 'clients', 'portfolio', 'reviews', 'socials'],
                  analytics: activeFreelancer.analytics || {
                    totalViews: 0,
                    totalInquiries: 0,
                    conversionRate: 0,
                    viewsHistory: [],
                  ...
                  id: activeRole,
                  fullName: userEmail ? (userEmail.split('@')[0].charAt(0).toUpperCase() + userEmail.split('@')[0].slice(1)) : 'Creative User',
                  username: 'creative_' + (activeRole || 'user').slice(0, 8),
                  category: 'photography',
                  hourlyRate: 3500,
                  bio: 'Welcome to my Creative Dashboard on Talanta Hub. Complete your portfolio details to highlight your work to potential clients.',
                  location: 'Nairobi County',
                  skills: ['Photography', 'Videography', 'Creative Direction'],
                  email: userEmail,
                  phone: '',
                  whatsapp: '',
                  avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200',
                  coverUrl: 'https://images.unsplash.com/photo-1579783902614-a3fb3927b675?auto=format&fit=crop&q=80&w=1200',
                  isClientOnly: false,
                  isPublic: true,
                  subscribedCategories: ['photography', 'videography'],
                  portfolio: [],
                  categorySections: [],
                  unlockedJobIds: [],
                  notificationCount: 0,
                  ...
                  offers: [],
                  requestedCalls: [],
                  layoutOrder: ['bio', 'stats', 'skills', 'clients', 'portfolio', 'reviews', 'socials'],
                  analytics: {
                    totalViews: 0,
                    totalInquiries: 0,
                    conversionRate: 0,
                ...
                return (
                  <Dashboard
                    ...
                    onOpenBecomeCreativeModal={() => {
                      // Handled inside Dashboard welcome onboarding and Settings/Password tab
                    }}
                  />
                );
              })()}
          ...
          {currentTab === 'terms' && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <TermsAndConditions onBackToHome={() => handleTabChange('home')} />
            </motion.div>
          )}
      ...
      {/* UNSAVED CHANGES REMINDER MODAL */}
      <AnimatePresence>
        {showUnsavedModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
            <motion.div
              ...
              className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 sm:p-8 max-w-md w-full text-center space-y-5"
            >
              <div className="h-14 w-14 bg-amber-100 text-amber-600 rounded-2xl flex items-center justify-center mx-auto border border-amber-200 shadow-sm animate-bounce">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-600 leading-relaxed font-medium">
                  You have draft updates or unpublished edits on your Dashboard. Would you like to save and publish your changes before leaving, or discard them?
                </p>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  className="w-full py-3 px-4 bg-gradient-to-r from-emerald-500 via-indigo-600 to-purple-600 hover:from-emerald-600 hover:to-purple-700 text-white rounded-xl font-black text-xs uppercase tracking-wider transition-all shadow-md hover:shadow-lg cursor-pointer active:scale-95 flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl font-bold text-xs transition-all cursor-pointer border border-rose-200 active:scale-95 flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="w-full py-2 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs transition-all cursor-pointer border border-slate-200 active:scale-95"
                >
                  Stay on Dashboard
                </button>
              </div>
      ...
      >
        ...
        <div className="relative z-10 flex flex-wrap justify-center gap-3.5 text-white font-extrabold text-[7px] sm:text-[8px] lg:text-[10px]" style={{ fontFamily: "Helvetica, 'Helvetica Neue', Arial, sans-serif" }}>
          <button onClick={() => handleTabChange('home')} className="hover:text-indigo-200 transition-colors cursor-pointer">Home</button>
          <span className="text-white/60">&bull;</span>
          <button onClick={() => handleTabChange('browse')} className="hover:text-indigo-200 transition-colors cursor-pointer">Explore Talents</button>
          <span className="text-white/60">&bull;</span>
          <button onClick={() => handleTabChange('jobs')} className="hover:text-indigo-200 transition-colors cursor-pointer">Job Market</button>
          <span className="text-white/60">&bull;</span>
          <button onClick={() => handleTabChange('terms')} className="hover:text-indigo-200 transition-colors cursor-pointer">Terms & Conditions</button>
        </div>
