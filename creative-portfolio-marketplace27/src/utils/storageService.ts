 ...
 * Uses a global cache and attempts signed URL creation for private buckets before falling back to public URL.
 */
  ...
  try {
    ...
    if (!error && data?.signedUrl) {
      signedUrlsCache[trimmed] = {
        url: data.signedUrl,
        expiresAt: Date.now() + (60 * 60 * 24 * 6.5 * 1000)
      };
      return data.signedUrl;
    }
  } catch (err) {
    console.warn('Error creating signed url from Supabase storage:', err);
  }
