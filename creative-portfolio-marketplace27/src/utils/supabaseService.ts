...
import { FreelancerProfile, PortfolioItem, FeedPost, Job, Review, CreativeCategory, CategorySection, Offer, CallRequest } from '../types';
 ...
 *   hired_creative_id text,
 *   is_completed boolean default false,
 *   created_at timestamp with time zone default timezone('utc'::text, now()) not null
 * );
 * 
 * -- Safe column migrations (Run if jobs table already existed before update):
 * alter table public.jobs add column if not exists unlock_count integer default 0;
 * alter table public.jobs add column if not exists unlock_price_ksh integer default 50;
 * alter table public.jobs add column if not exists hired_creative_id text;
 * alter table public.jobs add column if not exists is_completed boolean default false;
 * 
 ...
 * drop policy if exists "Allow public read access to portfolio items" on public.portfolio_items;
 * create policy "Allow public read access to portfolio items" on public.portfolio_items for select using (true);
 * drop policy if exists "Allow users to manage their own portfolio items" on public.portfolio_items;
 * create policy "Allow users to manage their own portfolio items" on public.portfolio_items for all using (auth.uid() = profile_id);
 * 
 * -- Policies for public.feed_posts
 * drop policy if exists "Allow public read access to feed posts" on public.feed_posts;
 * create policy "Allow public read access to feed posts" on public.feed_posts for select using (true);
 * drop policy if exists "Allow users to manage their own feed posts" on public.feed_posts;
 * create policy "Allow users to manage their own feed posts" on public.feed_posts for all using (auth.uid() = profile_id);
 * 
 ...
 * drop policy if exists "Allow anyone to insert jobs" on public.jobs;
 * create policy "Allow anyone to insert jobs" on public.jobs for insert with check (true);
 * drop policy if exists "Allow users to manage their own jobs" on public.jobs;
 * create policy "Allow users to manage their own jobs" on public.jobs for all using (auth.uid() = user_id);
 * 
 ...
 * create policy "Allow users to manage their own reviews" on public.reviews for all using (auth.uid() = reviewer_id);
 */
...
export function mapProfileFromDB(dbRow: any, dbPortfolio: any[] = [], dbPosts: any[] = []): FreelancerProfile {
  ...
  // Deduplicate portfolio items by ID and by title + image_url
  const seenPortfolioKeys = new Set<string>();
  const uniquePortfolio: PortfolioItem[] = [];
  for (const item of dbPortfolio) {
    const titleKey = (item.title || '').trim().toLowerCase();
    const imgKey = (item.image_url || '').trim().toLowerCase();
    const idKey = item.id ? String(item.id).trim() : '';
    
    const comboKey = `${titleKey}::${imgKey}`;
    if (idKey && seenPortfolioKeys.has(`id::${idKey}`)) continue;
    if (comboKey !== '::' && seenPortfolioKeys.has(`combo::${comboKey}`)) continue;

    if (idKey) seenPortfolioKeys.add(`id::${idKey}`);
    if (comboKey !== '::') seenPortfolioKeys.add(`combo::${comboKey}`);

    uniquePortfolio.push({
      id: item.id || `p_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
  ...
  return {
    ...
    whatsapp: dbRow.whatsapp || undefined,
...
export function mapProfileToDB(profile: FreelancerProfile): any {
  const categorySections = (profile.categorySections || []).filter(
    (sec: any) => sec.category !== '__notable_clients__' && sec.category !== '__requested_calls__'
  );
...
export const PROFILE_PUBLIC_COLUMNS = 'id, username, full_name, title, avatar_url, cover_url, bio, location, hourly_rate, category, skills, theme, layout_order, category_sections, reviews, analytics, subscribed_categories, notification_count, unread_messages_count, email, phone, whatsapp, wallet_balance_ksh, unlocked_job_ids, is_public, created_at';
...
/**
 * Loads a single profile from Supabase by user ID or email.
 */
export async function loadSingleProfileFromSupabase(identifier: string): Promise<FreelancerProfile | null> {
  try {
    const isEmail = identifier.includes('@');
    console.log(`[Auth DB] Loading single profile from Supabase by ${isEmail ? 'email' : 'ID'}: ${identifier}`);
    ...
    if (error || !profile) {
      console.log(`[Auth DB] No profile found in Supabase for ${identifier}`);
      return null;
    }
    ...
    const userReviews = dbReviews
      .filter((r: any) => r.reviewed_user_id === profile.id)
      .map((r: any) => ({
        id: r.id,
        authorName: r.reviewer_name,
        authorRole: r.reviewer_role,
    ...
    const mapped = mapProfileFromDB(profile, portfolioItems || [], feedPosts || []);
    mapped.reviews = userReviews;
    const resolved = await resolveProfileUrls(mapped);
    console.log(`[Auth DB] Successfully loaded single profile from Supabase for ${identifier}: ${resolved.fullName} (@${resolved.username})`);
    return resolved;
  } catch (err) {
    console.warn(`[Auth DB] Exception loading single profile from Supabase for ${identifier}:`, err);
    return null;
  }
}
...
export async function savePortfolioItems(profileId: string, items: PortfolioItem[]): Promise<boolean> {
  ...
  try {
    ...
    for (const item of items) {
      const titleKey = (item.title || '').trim().toLowerCase();
      const imgKey = (item.imageUrl || '').trim().toLowerCase();
      const comboKey = `${titleKey}::${imgKey}`;
      if (comboKey !== '::' && seen.has(comboKey)) continue;
      if (comboKey !== '::') seen.add(comboKey);
    ...
    const dbItems = uniqueItems.map(item => ({
...
export async function upsertJobInSupabase(job: Job, userId?: string | null): Promise<boolean> {
  ...
  try {
    ...
    // Automatic fallback if columns like 'hired_creative_id' do not exist in existing table schema cache
    if (error && (error.code === 'PGRST204' || error.message?.includes('column') || error.message?.includes('schema cache'))) {
      console.warn(
        '⚠️ Missing column in Supabase jobs table. Please run the following in your Supabase SQL Editor:\n' +
        'ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS hired_creative_id text;\n' +
        'ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS is_completed boolean DEFAULT false;\n' +
        'ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS unlock_count integer DEFAULT 0;\n' +
        'ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS unlock_price_ksh integer DEFAULT 50;'
      ...
      // Strip missing columns and retry upsert
      const fallbackJob = { ...dbJob };
      delete fallbackJob.hired_creative_id;
      delete fallbackJob.is_completed;
      delete fallbackJob.unlock_count;
      delete fallbackJob.unlock_price_ksh;
...
export async function fetchJobUnlocksFromSupabase(jobId: string): Promise<any[]> {
  if (!jobId) return [];

  const localUnlocks = getLocalContactUnlocks().filter((u: any) => u.jobId === jobId || u.job_id === jobId);

  if (!isValidUUID(jobId)) {
    return localUnlocks.sort((a, b) => new Date(b.createdAt || b.created_at || 0).getTime() - new Date(a.createdAt || a.created_at || 0).getTime());
  }
  ...
  try {
    ...
    if (!error && data) {
      const dbUnlocks = data.map((item: any) => ({
        id: item.id,
      ...
      const combined = [...dbUnlocks];
      for (const loc of localUnlocks) {
        if (!combined.some(c => c.id === loc.id || (c.buyerId === loc.buyerId && c.createdAt === loc.createdAt))) {
          combined.push({
            id: loc.id || generateUUID(),
            buyerId: loc.buyerId,
            creativeId: loc.creativeId,
            jobId: loc.jobId,
            amount: loc.amount,
            paymentStatus: loc.paymentStatus,
    ...
    console.warn('Error fetching job unlocks from Supabase:', err);
  }
...
/**
 * ============================================================================
 * UNIFIED ONBOARDING & AUTHENTICATION SERVICE
 * ============================================================================
 * Shared post-authentication handler for Email/Password, Google OAuth, and
 * session restoration. Guarantees 1:1 user accounts, Creative Dashboard profiles,
 * and unified onboarding across all auth providers.
 */
...
export async function ensureUserProfile(
  ...
  inMemoryList: FreelancerProfile[] = []
): Promise<{ profile: FreelancerProfile; wasCreated: boolean }> {
  const cleanEmail = (options.email || '').toLowerCase().trim();
  const cleanUserId = (options.userId || '').trim();

  console.log(`[Auth Flow] Unified post-auth processing for User ID: "${cleanUserId}", Email: "${cleanEmail}", Provider: "${options.authProvider || 'email'}"`);

  // 1. Search in-memory profiles list first
  let existing: FreelancerProfile | undefined = inMemoryList.find(f => 
    (cleanUserId && f.id === cleanUserId) ||
    (cleanEmail && f.email?.toLowerCase().trim() === cleanEmail)
  );

  if (existing) {
    console.log(`[Auth Flow] [RETRIEVED] Existing profile found in memory list for ${cleanEmail || cleanUserId}: "${existing.fullName}" (${existing.id})`);
  } else {
    // 2. Query Supabase Database for existing profile
    try {
      if (cleanUserId) {
        existing = (await loadSingleProfileFromSupabase(cleanUserId)) || undefined;
      }
      if (!existing && cleanEmail) {
        existing = (await loadSingleProfileFromSupabase(cleanEmail)) || undefined;
      }
    } catch (err) {
      console.warn('[Auth Flow] Exception querying Supabase DB for existing profile:', err);
    }
  }

  // 3. If User Already Exists -> Never create another dashboard, profile or account!
  if (existing) {
    console.log(`[Auth Flow] [RETRIEVED] Located existing user profile: Name: "${existing.fullName}", Username: "@${existing.username}", ID: "${existing.id}". Reusing Creative Dashboard.`);

    let needsDbSync = false;
    // Align profile ID if auth ID changed or was unlinked
    if (cleanUserId && existing.id !== cleanUserId) {
      console.log(`[Auth Flow] Updating profile ID link from ${existing.id} to ${cleanUserId}`);
      existing.id = cleanUserId;
      needsDbSync = true;
    }

    if (existing.isClientOnly) {
      existing.isClientOnly = false;
      needsDbSync = true;
    }

    if (cleanEmail && (!existing.email || existing.email.toLowerCase().trim() !== cleanEmail)) {
      existing.email = cleanEmail;
      needsDbSync = true;
    }

    if (needsDbSync) {
      upsertFreelancerProfile(existing).catch(err => 
        console.warn('[Auth Flow] Non-blocking notice updating matched profile in DB:', err)
      );
    }

    return { profile: existing, wasCreated: false };
  }

  // 4. If No Dashboard/Profile Exists -> Create Creative Dashboard profile using onboarding logic!
  console.log(`[Auth Flow] [CREATED] No existing profile found for ${cleanEmail || cleanUserId}. Initializing new Creative Dashboard profile via ${options.authProvider || 'email'}.`);

  const rawName = options.fullName || options.extraMeta?.fullName || (cleanEmail ? cleanEmail.split('@')[0].replace(/[._-]/g, ' ') : 'Creative User');
  ...
  const usernameBase = (formattedName.toLowerCase().replace(/[^a-z0-9_]/g, '_') || 'creative');
  const uniqueUsername = `${usernameBase}_${Math.floor(Math.random() * 8999 + 1000)}`;

  const primaryCategory = (options.extraMeta?.categories && options.extraMeta.categories[0]) || 'photography';
  ...
  const newProfile: FreelancerProfile = {
    ...
    hourlyRate: options.extraMeta?.hourlyRate || 3500,
    bio: options.extraMeta?.bio || `Welcome to my Talanta Hub profile—explore my work and creative journey.`,
    location: options.extraMeta?.location || 'Nairobi County',
    skills: options.extraMeta?.skills || ['Photography', 'Videography', 'Creative Direction'],
    theme: 'slate',
    email: cleanEmail,
    phone: options.extraMeta?.phone || '',
    whatsapp: options.extraMeta?.whatsapp || '',
    avatarUrl: options.avatarUrl || options.extraMeta?.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200',
    coverUrl: options.coverUrl || options.extraMeta?.coverUrl || 'https://images.unsplash.com/photo-1579783902614-a3fb3927b675?auto=format&fit=crop&q=80&w=1200',
    isPublic: true,
    isClientOnly: false,
    layoutOrder: ['hero', 'categories', 'gallery', 'analytics', 'reviews', 'contact', 'offers'],
    subscribedCategories: options.extraMeta?.subscribedCategories || options.extraMeta?.categories || ['photography', 'videography'],
    portfolio: options.extraMeta?.portfolio || [],
    categorySections: options.extraMeta?.categorySections || [
      {
    ...
    unlockedJobIds: options.extraMeta?.unlockedJobIds || [],
    notificationCount: 0,
    unreadMessagesCount: 0,
    notableClients: options.extraMeta?.notableClients || [],
    offers: options.extraMeta?.offers || [],
    requestedCalls: options.extraMeta?.requestedCalls || [],
    feedPosts: [
      {
        id: `f_post_${Date.now()}`,
        caption: `Welcome to my Talanta Hub profile!\n\nI'm excited to share my creative portfolio and collaborate on projects across Kenya. Feel free to connect and explore my work!`,
        likes: 0,
  ...
  try {
    ...
    if (newProfile.feedPosts && newProfile.feedPosts.length > 0) {
      await saveFeedPosts(newProfile.id, newProfile.feedPosts);
    }
    console.log(`[Auth Flow] [CREATED] Successfully persisted new Creative Dashboard profile in Supabase DB: "${newProfile.fullName}" (@${newProfile.username}, ID: ${newProfile.id}).`);
  } catch (err) {
    console.error('[Auth Flow] Error persisting newly created profile to Supabase DB:', err);
  }
