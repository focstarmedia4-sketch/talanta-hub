...
import { loadSingleProfileFromSupabase, ensureUserProfile } from '../utils/supabaseService';
...
interface CreativeAuthPortalProps {
  ...
  onUpdateProfile?: (profile: FreelancerProfile) => void;
...
export function CreativeAuthPortal({
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Process post-authentication via unified onboarding service
      ...
      console.log(`[Auth UI] Setting active role to ${profile.id} (${profile.fullName}). Was Created? ${wasCreated}`);
      ...
      setSigninEmail('');
      setSigninPassword('');
      setAuthSuccess(wasCreated ? 'Account created and profile initialized!' : 'Logged in successfully!');
      onChangeTab('dashboard');
      onClose();
    } catch (err: any) {
  ...
  const handleGoogleAccountClick = async (email: string, name: string) => {
    ...
    console.log(`[Auth UI] Google sign-in completed for ${cleanEmail}. Profile ID: ${profile.id}, Created? ${wasCreated}`);
    if (onUpdateProfile) {
      onUpdateProfile(profile);
    }
    onChangeRole(profile.id);
