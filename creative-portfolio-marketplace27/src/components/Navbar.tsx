...
import { ensureUserProfile } from '../utils/supabaseService';
...
const NAV_CATEGORIES: { id: CreativeCategory; label: string; icon: string }[] = [
  ...
  { id: 'dops', label: 'Directors of Photography', icon: '📹' },
  ...
  { id: 'filmdirectors', label: 'Film Directors', icon: '🎬' },
  { id: 'fineartist', label: 'Fine Artists', icon: '🎨' },
  { id: 'florists', label: 'Florists And Floral Designers', icon: '💐' },
  { id: 'framingmoulds', label: 'Framing & Photo Moulds', icon: '🖼️' },
  { id: 'design', label: 'Graphic Design', icon: '🎨' },
  { id: 'illustration', label: 'Illustration', icon: '✏️' },
  { id: 'interiordesign', label: 'Interior Design', icon: '🏠' },
  { id: 'craneoperators', label: 'Jib & Crane Operators', icon: '🏗️' },
  { id: 'lightingdesigners', label: 'Lighting Designers', icon: '💡' },
  { id: 'livestream', label: 'Livestream & Broadcast', icon: '📡' },
...
export default function Navbar({
  ...
  const [catDropdownCoords, setCatDropdownCoords] = useState<{ top: number; left: number } | null>(null);
  const [notifDropdownCoords, setNotifDropdownCoords] = useState<{ top: number; right: number } | null>(null);
  const [accountDropdownCoords, setAccountDropdownCoords] = useState<{ top: number; right: number } | null>(null);

  const toggleCategoryDropdown = () => {
    if (!showCategoryDropdown && categoryDropdownRef.current) {
      const rect = categoryDropdownRef.current.getBoundingClientRect();
      const leftPos = Math.max(8, Math.min(rect.left, window.innerWidth - 270));
      setCatDropdownCoords({ top: rect.bottom + 6, left: leftPos });
    }
    setShowCategoryDropdown(!showCategoryDropdown);
  };

  const toggleNotificationDropdown = () => {
    if (!showNotificationDropdown && notificationDropdownRef.current) {
      const rect = notificationDropdownRef.current.getBoundingClientRect();
      setNotifDropdownCoords({ top: rect.bottom + 6, right: Math.max(8, window.innerWidth - rect.right) });
    }
  ...
  const toggleAccountDropdown = () => {
    if (!showAccountDropdown && accountDropdownRef.current) {
      ...
      setAccountDropdownCoords({ top: rect.bottom + 6, right: Math.max(8, window.innerWidth - rect.right) });
    }
  ...
  useEffect(() => {
    if (isLoggedIn && showJoinModal) {
      console.log('[Auth Navbar] User authenticated, automatically closing auth portal modal');
      setShowJoinModal(false);
    }
  }, [isLoggedIn, showJoinModal]);

  const handleGoogleAccountClick = async (email: string, name: string) => {
    setAuthError(null);
    setAuthSuccess(null);
    const cleanEmail = email.toLowerCase().trim();
    console.log(`[Auth Navbar] Google account selected: ${name} (${cleanEmail})`);

    const googleUserId = `google_${cleanEmail.replace(/[^a-z0-9]/g, '_')}`;
    ...
    console.log(`[Auth Navbar] Google sign-in finished for ${cleanEmail}. Profile ID: ${profile.id}, Created? ${wasCreated}`);
    if (onUpdateProfile) {
      onUpdateProfile(profile);
    }
    onChangeRole(profile.id);
    onLogin(profile.id);
    onChangeTab('dashboard');
    setShowGoogleChooser(false);
    setShowJoinModal(false);
    setAuthSuccess(wasCreated ? `Registered & signed in with Google as ${name}!` : `Signed in with Google as ${name}!`);
  };

  const unreadNotifications = notifications.filter(n => !n.read);
  const selectedFreelancer = freelancers.find(
    f => f.id === activeRole || (f.email && f.email.toLowerCase().trim() === activeRole.toLowerCase().trim())
  );
  const isFreelancer = isLoggedIn && (activeRole !== 'client' || !!selectedFreelancer);
  ...
  return (
    <header className="bg-[#87cefa] border-b border-white/20 sticky top-0 z-[100] shadow-md">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 min-h-12 flex items-center justify-between gap-2 sm:gap-4 overflow-visible">
        ...
        <div 
          ...
          className="flex items-center justify-center cursor-pointer shrink-0 hover:opacity-95 transition-opacity h-7 max-h-7 w-auto max-w-[110px] sm:max-w-[130px] rounded-lg overflow-hidden my-auto self-center -translate-y-1 sm:translate-y-0"
        >
          <img 
            ...
            className="h-7 max-h-7 w-auto max-w-full object-contain block"
            loading="eager"
            decoding="async"
            fetchPriority="high"
        ...
        {/* 2. UNIFIED HORIZONTAL SCROLL CONTAINER FOR NAV & ACTION BUTTONS */}
        <div className="flex items-center justify-start sm:justify-center flex-1 gap-2 sm:gap-2.5 mx-1 sm:mx-0 overflow-x-auto custom-scrollbar flex-nowrap shrink min-w-0 max-w-full py-0.5 px-0.5 my-auto">
          {/* NAVIGATION LINKS */}
          <nav className="flex items-center gap-1.5 sm:gap-2 shrink-0 my-auto">
            {/* HOME BUTTON */}
            <button
              onClick={() => onChangeTab('home')}
              className={`h-7 flex items-center gap-1 px-2.5 rounded-md text-[10px] font-semibold capitalize tracking-normal transition-all cursor-pointer border shrink-0 whitespace-nowrap self-center ${
                currentTab === 'home' 
            ...
            <div ref={categoryDropdownRef} className="relative shrink-0 self-center">
              <button
                onClick={toggleCategoryDropdown}
                className={`h-7 flex items-center gap-1 px-2.5 rounded-md text-[10px] font-semibold capitalize tracking-normal transition-all cursor-pointer border shrink-0 whitespace-nowrap ${
                  currentTab === 'browse' 
                    ? 'bg-indigo-950 text-white border-indigo-950 shadow-xs' 
                    : 'bg-white/70 hover:bg-white text-indigo-950 hover:text-indigo-950 border-indigo-900/5'
                }`}
              ...
              <AnimatePresence>
                {showCategoryDropdown && (
                  <motion.div
                     ...
                     style={{
                       position: 'fixed',
                       top: catDropdownCoords ? `${catDropdownCoords.top}px` : undefined,
                       left: catDropdownCoords ? `${catDropdownCoords.left}px` : undefined,
                     }}
                     className="w-64 rounded-xl bg-white/95 backdrop-blur-md border border-slate-200/80 shadow-2xl p-1 z-[9999] grid grid-cols-1 gap-0.5 max-h-72 overflow-y-auto"
                  >
                    <button
                      onClick={() => {
            ...
            <button
              ...
              className="h-7 flex items-center gap-1 px-2.5 bg-white/70 hover:bg-white text-indigo-950 hover:text-indigo-950 border border-indigo-900/5 rounded-md text-[10px] font-semibold capitalize tracking-normal transition-all cursor-pointer shrink-0 whitespace-nowrap self-center"
            >
          ...
          {/* Right side actions and user session controls */}
          <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 whitespace-nowrap my-auto">
          ...
          {isUserLoggedIn ? (
            <div className="flex items-center gap-2.5">
              <>
                <button
                  ...
                  className={`h-7 flex items-center gap-1 px-2 rounded-md text-[10px] font-semibold uppercase tracking-wider shadow-xs transition-all cursor-pointer border self-center ${
                ...
                <div ref={notificationDropdownRef} className="relative self-center">
                  <button
                  ...
                  <AnimatePresence>
                    {showNotificationDropdown && (
                      <motion.div
                        ...
                        style={{
                          position: 'fixed',
                          top: notifDropdownCoords ? `${notifDropdownCoords.top}px` : undefined,
                          right: notifDropdownCoords ? `${notifDropdownCoords.right}px` : undefined,
                        }}
                        className="w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 z-[9999] text-white"
                      >
            ...
            <div className="flex items-center gap-2 self-center">
              <button
                ...
                className="h-7 flex items-center gap-1 px-2 bg-[#517f9b] hover:bg-[#436b84] text-white border border-[#4e7994] rounded-md text-[10px] font-extrabold uppercase tracking-wider shadow-xs transition-all cursor-pointer hover:scale-102 active:scale-98 self-center"
              >
                <UserPlus className="h-3 w-3 text-indigo-100" />
                <span>Creative Portal</span>
              </button>
            </div>
          )}

          {/* Vertical Separator */}
          {isUserLoggedIn && <div className="h-5 w-px bg-indigo-900/20 self-center" />}

          {/* Active Session & Account Customizer Dropdown */}
          {isUserLoggedIn && (
            <div ref={accountDropdownRef} className="relative self-center">
              <button
              ...
              >
                <div className="h-5 w-5 rounded-full overflow-hidden border border-indigo-900/20 shrink-0 bg-slate-800">
                  {selectedFreelancer?.avatarUrl ? (
                    <img 
                      src={selectedFreelancer.avatarUrl} 
                ...
                <span className="max-w-[100px] truncate uppercase tracking-wider text-[11px]">
                  {selectedFreelancer?.fullName || (activeRole === 'client' ? 'Client' : 'My Account')}
                </span>
              ...
              <AnimatePresence>
                {showAccountDropdown && (
                  <motion.div
                    ...
                    style={{
                      position: 'fixed',
                      top: accountDropdownCoords ? `${accountDropdownCoords.top}px` : undefined,
                      right: accountDropdownCoords ? `${accountDropdownCoords.right}px` : undefined,
                    }}
                    className="w-72 bg-white rounded-2xl border border-slate-100 shadow-2xl p-4 z-[9999] space-y-4 text-left text-slate-800"
                  >
