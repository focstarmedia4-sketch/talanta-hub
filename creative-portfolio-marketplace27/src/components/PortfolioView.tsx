...
export default function PortfolioView({ 
  ...
  useEffect(() => {
    ...
    const itemParam = searchParams.get('item') || searchParams.get('itemId') || initialItemId;

    if (itemParam && profile.portfolio && profile.portfolio.length > 0) {
      const targetId = String(itemParam).trim().toLowerCase();
      
      if (autoOpenedItemRef.current === targetId) {
        return;
      }

      const match = profile.portfolio.find(item => String(item.id).trim().toLowerCase() === targetId);
      if (match) {
        autoOpenedItemRef.current = targetId;
        ...
        const scrollTimer = setTimeout(() => {
          const el = document.getElementById(`portfolio-item-${match.id}`) || document.getElementById('gallery') || document.getElementById('works');
          if (el) {
