...
export function CreativeAuthPortal({
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // Fallback for local profile match
        ...
        setAuthError('No creative profile found with those credentials. Please check your details or Sign Up as a Creative.');
        return;
      }
      ...
      if (found) {
        ...
        setAuthError('No creative profile found for this account. Please sign up as a Creative.');
      }
    } catch (err: any) {
      const localMatch = freelancers.find(f => f.email?.toLowerCase().trim() === signinEmail.toLowerCase().trim());
      if (localMatch) {
        ...
        setAuthError(err?.message || 'Login failed. Please verify your credentials or Sign Up as a Creative.');
      }
    }
  };
  ...
  const handleJoinSubmit = async (e: React.FormEvent) => {
    ...
    // Try Supabase auth signup in background, bypassing any email rate limit / confirmation requirements
    try {
      ...
      console.warn("Supabase auth signup notice (email verification bypassed):", sbErr);
    }
    ...
    // Move straight to Profile Setup step without requiring email verification
    setIsProfileSetupStep(true);
    setAuthSuccess('Account created! Email verification bypassed—complete your profile below or skip to get started.');
  };
