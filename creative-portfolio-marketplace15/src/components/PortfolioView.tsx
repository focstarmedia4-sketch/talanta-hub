...
export default function PortfolioView({ 
  ...
  useEffect(() => {
    if (initialItemId && profile.portfolio) {
      const match = profile.portfolio.find(item => item.id === initialItemId);
      if (match) {
  ...
  const handleShareCatalogueItem = (item: PortfolioItem) => {
    const shareUrl = `${window.location.origin}${window.location.pathname}#creative=${profile.username || profile.id}&item=${item.id}`;
                    ...
                    <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-300 rounded-xl transition-all cursor-pointer font-bold text-xs flex items-center gap-1 border border-indigo-200 dark:border-indigo-800"
                          title="Share direct link to this catalogue item"
                        >
                          <Share2 className="h-3.5 w-3.5" />
                          <span className="text-[10px] uppercase tracking-wider font-extrabold">Share</span>
                        </button>
                        <button
                          onClick={() => setSelectedGalleryItem(null)}
