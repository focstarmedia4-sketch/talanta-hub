...
import { loadSingleProfileFromSupabase } from '../utils/supabaseService';
...
export function CreativeAuthPortal({
  ...
  const [bio, setBio] = useState('Welcome to my Talanta Hub profile—explore my work and creative journey.');
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    console.log(`[Auth UI] Attempting sign-in for email: ${cleanEmail}`);
    ...
    try {
      ...
      if (error) {
        console.warn(`[Auth UI] Supabase signInWithPassword returned error:`, error.message);

        // Fallback check for local profile match
        const localMatch = freelancers.find(f => f.email?.toLowerCase().trim() === cleanEmail);
        if (localMatch) {
          console.log(`[Auth UI] Local profile match found for ${cleanEmail}: ${localMatch.id}`);
          onChangeRole(localMatch.id);
          onLogin(localMatch.id);
          setSigninEmail('');
          setSigninPassword('');
        ...
        // Check if user account was registered via Google OAuth
        if (error.message.toLowerCase().includes('invalid login credentials')) {
          try {
            ...
            if (dbProfiles && dbProfiles.length > 0) {
              setAuthError('An account with this email exists. If you previously registered using Google, please click "Continue with Google" to sign in.');
              return;
            }
          } catch (chkErr) {
            console.warn('[Auth UI] Notice checking profiles on login fail:', chkErr);
          }
        }
      ...
      console.log(`[Auth UI] Password authentication succeeded for user ${userId}`);

      // Look up if this user has a Creative profile
      const creativeProfile = freelancers.find(f => f.id === userId || (f.email && f.email.toLowerCase().trim() === cleanEmail));

      const activeRoleId = creativeProfile ? creativeProfile.id : (userId || cleanEmail);
      console.log(`[Auth UI] Setting active role to ${activeRoleId}`);

      onChangeRole(activeRoleId);
      onLogin(activeRoleId);
      setSigninEmail('');
      setSigninPassword('');
      setAuthSuccess('Logged in successfully!');
      onChangeTab('dashboard');
      ...
      console.error(`[Auth UI] Sign-in exception:`, err);
  ...
  const handleForgotPasswordSubmit = (e: React.FormEvent) => {
    ...
    const found = freelancers.find(f => f.email?.toLowerCase().trim() === forgotEmail.trim().toLowerCase());
    if (found) {
  ...
  const handleJoinSubmit = async (e: React.FormEvent) => {
    ...
    const cleanEmail = creativeEmail.trim().toLowerCase();
    ...
    console.log(`[Auth UI] Checking if email ${cleanEmail} is already registered...`);

    // 1. Check in-memory freelancers list
    const existingLocal = freelancers.find(f => f.email?.toLowerCase().trim() === cleanEmail);
    if (existingLocal) {
      console.warn(`[Auth UI] Email ${cleanEmail} already registered in local profile list.`);
      if (existingLocal.password === 'google_oauth' || existingLocal.password === 'google_oauth_bypass') {
        setAuthError("This email is already registered using Google. Please click 'Continue with Google' to sign in.");
      } else {
        setAuthError("An account with this email address already exists. Please Sign In with your password instead of signing up.");
      }
      return;
    }

    // 2. Check Supabase profiles table directly
    try {
      ...
      if (dbProfile) {
        console.warn(`[Auth UI] Email ${cleanEmail} already registered in Supabase profiles table.`);
        setAuthError("An account with this email address already exists. Please Sign In with your password instead of signing up.");
        return;
      }
    } catch (dbCheckErr) {
      console.warn("[Auth UI] Notice checking database for existing profile:", dbCheckErr);
    }
    ...
    try {
      ...
      if (signUpError) {
        console.warn(`[Auth UI] Supabase auth.signUp error:`, signUpError.message);
        const errMsg = signUpError.message.toLowerCase();

        if (errMsg.includes('already registered') || errMsg.includes('already exists') || signUpError.status === 400) {
          if (errMsg.includes('google') || errMsg.includes('oauth')) {
            setAuthError("This email address is already registered using Google. Please click 'Continue with Google' to sign in.");
          } else {
            setAuthError("An account with this email address already exists. Please Sign In with your password instead of signing up.");
          }
          return;
      ...
      // Check for obfuscated duplicate email user (user with empty identities in Supabase)
      if (authData.user && authData.user.identities && authData.user.identities.length === 0) {
        console.warn(`[Auth UI] Supabase returned user with empty identities (duplicate email detection).`);
        setAuthError("An account with this email address already exists. Please Sign In with your password instead of signing up.");
        return;
      }

      console.log(`[Auth UI] Supabase signup succeeded for email: ${cleanEmail}`);
    } catch (sbErr: any) {
      console.error("[Auth UI] Supabase auth signup exception:", sbErr);
      setAuthError(sbErr?.message || "Could not complete account creation.");
      return;
    }
    ...
    // Move straight to Profile Setup step
    setIsProfileSetupStep(true);
    setAuthSuccess('Account created! Complete your creative profile details below to get started.');
  };
  ...
  const handleGoogleAccountClick = async (email: string, name: string) => {
    ...
    console.log(`[Auth UI] Google account selected: ${name} (${cleanEmail})`);

    // 1. Search in-memory freelancers list
    let existing = freelancers.find(f => f.email?.toLowerCase().trim() === cleanEmail);
    ...
    if (!existing) {
      ...
      try {
        ...
        if (dbProfile) {
          existing = await loadSingleProfileFromSupabase(dbProfile.id) || undefined;
        }
      } catch (err) {
        console.warn('[Auth UI] Notice checking Supabase profile on Google account choice:', err);
      }
    }

    if (existing) {
      console.log(`[Auth UI] Found existing profile for Google email ${cleanEmail}: ${existing.id}`);
      onChangeRole(existing.id);
      onLogin(existing.id);
      onChangeTab('dashboard');
      ...
      console.log(`[Auth UI] No existing profile for Google email ${cleanEmail}. Registering new profile.`);
      onJoinAsCreative({
        fullName: name,
            ...
            {isProfileSetupStep ? (
              ...
              <div className="space-y-6">
                <div className="text-center">
                  <h3 className="text-xs sm:text-sm md:text-base font-black text-slate-900 uppercase tracking-wider mb-1">Complete Your Profile</h3>
                  <p className="text-[9px] sm:text-xs text-slate-450">Please complete the details below to activate your professional dashboard</p>
                </div>
