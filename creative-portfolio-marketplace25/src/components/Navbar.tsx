...
export default function Navbar({
  ...
  const handleGoogleAccountClick = (email: string, name: string) => {
    ...
    if (found) {
      // User exists with creative profile!
      ...
      // Authenticate as User/Client account without creating a creative profile
      const googleUserId = `google_${email.toLowerCase().replace(/[^a-z0-9]/g, '_')}`;
      onChangeRole(googleUserId);
      onLogin(googleUserId);
      onChangeTab('dashboard');
      setShowGoogleChooser(false);
      setShowJoinModal(false);
      setAuthSuccess(`Signed in with Google as ${name}!`);
