...
export function CreativeAuthPortal({
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    // Check for dummy / demo account credentials shortcut
    if (
      signinEmail.toLowerCase().includes('dummy') || 
      signinEmail.toLowerCase().includes('demo') || 
      signinPassword === 'demo123456'
    ) {
    ...
    try {
      ...
      if (error) {
        // Fallback for local dummy check if Supabase error occurs
        const localMatch = freelancers.find(f => f.email?.toLowerCase().trim() === signinEmail.toLowerCase().trim());
        if (localMatch) {
  ...
  const handleInstantDemoLogin = () => {
    ...
    let dummy = freelancers.find(f => 
      f.id === 'dummy_creative_account' ||
      f.email?.toLowerCase().includes('dummy') || 
      f.email?.toLowerCase().includes('demo') || 
      f.username === 'dummy_creative' ||
      f.fullName.toLowerCase().includes('demo')
    );
    ...
    if (!dummy) {
      if (freelancers.length > 0) {
        ...
        const dummyProfile: FreelancerProfile = {
          id: 'dummy_creative_account',
          username: 'dummy_creative',
          fullName: 'Alex Mutua (Demo Creative)',
          title: 'Senior Creative Specialist & Photographer',
          avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200',
          coverUrl: 'https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&q=80&w=1200',
          bio: 'Demo creative profile for testing dashboard tools, portfolio management, job applications, and messaging.',
          location: 'Nairobi County',
          hourlyRate: 3500,
          category: 'photography',
          skills: ['Photography', 'Videography', 'Graphic Design', 'Branding'],
          theme: 'slate',
          email: 'dummy.creative@talantahub.co.ke',
          phone: '+254712345678',
          whatsapp: '+254712345678',
          subscribedCategories: ['photography', 'videography', 'design'],
          password: 'demo123456',
          walletBalanceKsh: 5000,
          unlockedJobIds: [],
          notificationCount: 2,
          ...
          portfolio: [
            {
              id: 'p_demo_1',
              title: 'Nairobi Fashion Week Showcase',
              description: 'Editorial photography captured live in Nairobi.',
          ...
          categorySections: [
            {
              ...
              customThumbnail: 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?auto=format&fit=crop&w=600&q=70',
              description: 'Commercial & fashion photography showcase',
              visible: true
            }
          ],
          analytics: {
            ...
            reachByCategory: [
              { category: 'photography', percentage: 70 },
              { category: 'videography', percentage: 30 }
            ]
          }
        };
        onJoinAsCreative({
          ...
          categories: ['photography', 'videography', 'design'],
          hourlyRate: dummyProfile.hourlyRate,
          ...
          whatsapp: dummyProfile.whatsapp,
          avatarUrl: dummyProfile.avatarUrl,
          coverUrl: dummyProfile.coverUrl,
          password: dummyProfile.password
        });
        dummy = dummyProfile;
      }
    }

    onChangeRole(dummy.id);
    onLogin(dummy.id);
    setAuthSuccess(`Logged in as ${dummy.fullName}!`);
    onChangeTab('dashboard');
    onClose();
  };
              ...
              <div className="space-y-6">
                ...
                {!forgotPasswordActive ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3.5 bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-200/80 rounded-2xl space-y-2 text-center shadow-xs">
                      <div className="flex items-center justify-center gap-1.5 text-indigo-700 font-extrabold text-xs">
                        <Sparkles className="h-4 w-4 text-indigo-600 animate-pulse" />
                        <span>Prompting changes? Use Demo Mode</span>
                      </div>
                      <p className="text-[11px] text-slate-600 font-medium">
                        Log in instantly as a pre-configured creative professional to test and prompt changes:
                      </p>
                      <button
                        type="button"
                        onClick={handleInstantDemoLogin}
                        className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-sm cursor-pointer flex items-center justify-center gap-2 hover:shadow-md active:scale-[0.99]"
                      >
                        <UserPlus className="h-4 w-4" />
                        <span>⚡ One-Click Demo Account Login</span>
                      </button>
                      <div className="text-[10px] text-slate-500 font-mono pt-0.5">
                        Demo Email: <span className="font-bold text-slate-700">dummy.creative@talantahub.co.ke</span> | Password: <span className="font-bold text-slate-700">demo123456</span>
                      </div>
                    </div>

                    <div className="relative flex py-1 items-center">
                      <div className="flex-grow border-t border-slate-150"></div>
                      <span className="flex-shrink mx-3 text-[10px] font-bold uppercase text-slate-400 tracking-wider">or sign in manually</span>
                      <div className="flex-grow border-t border-slate-150"></div>
                    </div>
