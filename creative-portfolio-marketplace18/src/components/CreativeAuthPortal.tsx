...
export function CreativeAuthPortal({
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // Fallback for local profile match or email rate limit / unconfirmed email bypass
        ...
        // Bypassing email verification / rate limit errors for instant sign in
        onChangeRole('client');
        onLogin('client');
        setSigninEmail('');
        setSigninPassword('');
      ...
      if (found) {
        ...
        // User account authenticated without a public creative profile
        const userId = data.user ? data.user.id : 'client';
        onChangeRole(userId);
        onLogin(userId);
        setSigninEmail('');
        setSigninPassword('');
