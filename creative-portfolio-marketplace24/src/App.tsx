...
export default function App() {
  ...
  useEffect(() => {
    ...
    const syncSessionState = async (session: any, eventType: string) => {
      ...
      console.log(`[Auth App] Session active for User ID: ${userId}, Email: ${userEmail}`);

      // 1. Synchronize/Link Profile 1:1 with Auth User ID
      let matchedProfile = activeFreelancers.find(
        f => f.id === userId || (userEmail && f.email?.toLowerCase().trim() === userEmail)
      );

      if (matchedProfile) {
        console.log(`[Auth App] In-memory profile match found for ${userEmail}: ${matchedProfile.fullName} (${matchedProfile.id})`);
        if (matchedProfile.id !== userId) {
          console.log(`[Auth App] Updating profile ID in memory & Supabase from ${matchedProfile.id} to ${userId}`);
          const updated = { ...matchedProfile, id: userId };
          setFreelancers(prev => deduplicateProfiles(prev.map(f => (f.id === matchedProfile!.id || (userEmail && f.email?.toLowerCase().trim() === userEmail)) ? updated : f)));
          activeFreelancers = activeFreelancers.map(f => (f.id === matchedProfile!.id || (userEmail && f.email?.toLowerCase().trim() === userEmail)) ? updated : f);
          upsertFreelancerProfile(updated).catch(e => console.warn('[Auth App] Error syncing updated profile ID:', e));
        }
      } else {
        console.log(`[Auth App] Searching Supabase profiles table for user ID ${userId} or email ${userEmail}`);
        try {
          const single = await loadSingleProfileFromSupabase(userId) || (userEmail ? await loadSingleProfileFromSupabase(userEmail) : null);
          if (single) {
            console.log(`[Auth App] Loaded profile from Supabase DB: ${single.fullName}`);
            if (single.id !== userId) {
              single.id = userId;
              upsertFreelancerProfile(single).catch(e => console.warn('[Auth App] Error syncing loaded profile ID:', e));
            }
            setFreelancers(prev => deduplicateProfiles([single, ...prev]));
            activeFreelancers = deduplicateProfiles([single, ...activeFreelancers]);
            matchedProfile = single;
          } else {
            console.log(`[Auth App] No creative profile found in database for ${userEmail}. Authenticated as Client.`);
          }
        } catch (err) {
          console.warn('[Auth App] Error querying profile from Supabase:', err);
        }
      }
      ...
      // 3. Drive redirection to dashboard on sign-in or OAuth redirect
      ...
      if (eventType === 'SIGNED_IN' || eventType === 'INITIAL_SESSION' || hasOAuthHash) {
        console.log(`[Auth App] Directing authenticated user ${userId} to dashboard tab. Creative Profile? ${!!matchedProfile}`);
  ...
  const handleJoinAsCreative = async (newCreative: {
    ...
    const newProfile: FreelancerProfile = {
      ...
      bio: newCreative.bio || 'Welcome to my Talanta Hub profile—explore my work and creative journey.',
    ...
    setFreelancers(prev => deduplicateProfiles([newProfile, ...prev]));
  ...
  if (sharedUsername) {
    ...
    const displayedProfile = {
      ...baseProfile,
      portfolio: selectedDeepProfile.portfolio && selectedDeepProfile.portfolio.length > 0 
          ...
          {currentTab === 'browse' && (
            ...
            >
              {sharedUsername && (() => {
                ...
                const displayedProf = {
                  ...baseProf,
                  portfolio: targetProfile.portfolio && targetProfile.portfolio.length > 0 
            ...
            >
              {(() => {
                ...
                const effectiveProfile: FreelancerProfile = activeFreelancer ? {
                  ...
                  id: activeRole,
                  fullName: userEmail ? userEmail.split('@')[0] : 'Client User',
                  username: 'client_' + (activeRole || 'user').slice(0, 8),
                  category: 'videography',
                  hourlyRate: 0,
                  bio: 'Client account on Talanta Hub',
