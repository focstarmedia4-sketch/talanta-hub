        ...
        return (
          <section key="categories" id="categories" className="space-y-6">
            <div className="space-y-1">
              <h2 className="font-hugh text-xs sm:text-xl md:text-3xl lg:text-4xl text-slate-900 dark:text-white tracking-normal normal-case">
                interact with me
              </h2>
            </div>
            ...
            <div className="space-y-4 max-h-[460px] overflow-y-auto pr-2 custom-scrollbar">
              {feedPosts.length === 0 ? (
                <div className="text-center py-8 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  ...
                  {isOwner && (
                    <p className="text-xs text-indigo-500 font-medium mt-1">Publish updates and creative posts from your Dashboard!</p>
                  )}
                </div>
              ) : (
                ...
                <AnimatePresence mode="popLayout">
                  {filteredWorks.map((item, idx) => (
                    <motion.div
                      ...
                      className="bg-[#90D5FF] border border-sky-400/40 rounded-lg overflow-hidden group flex flex-col justify-between shadow-xs cursor-pointer hover:border-indigo-600 hover:shadow-md hover:scale-[1.01] transition-all duration-300 relative"
                    >
                        ...
                        {displayedSlides.map((slide, sIdx) => {
                          ...
                          return (
                            <div
                              ...
                              className="relative w-full rounded-2xl overflow-hidden border-2 border-sky-400/40 cursor-pointer bg-[#90D5FF]/80 flex flex-col items-center justify-center group shadow-lg hover:border-indigo-600 hover:shadow-2xl transition-all duration-300"
                            ...
                            >
                              {isSlideVideo ? (
                                <div className="w-full aspect-video bg-slate-950 flex flex-col items-center justify-center relative">
                                  ...
                                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent flex flex-col items-center justify-center">
                                    <div className="bg-indigo-600 text-white p-3 rounded-full shadow-xl transform group-hover:scale-110 transition-transform">
                                      <svg className="h-6 w-6 fill-current ml-0.5" viewBox="0 0 24 24">
                                        <path d="M8 5v14l11-7z" />
                                      </svg>
                                    </div>
                                    <span className="mt-2 text-xs font-black uppercase tracking-wider text-white bg-slate-950/70 px-3 py-1 rounded-full border border-white/20">
                                      Click for Fullscreen Video
                                    </span>
                                  </div>
                                </div>
                              ) : (
                                <div className="w-full relative flex items-center justify-center p-2 sm:p-4 bg-sky-950/10">
                                  {/* Full size photo as uploaded */}
                                  <img
                                    src={slide}
                                    alt={`Gallery photo ${sIdx + 1}`}
                                    className="w-full h-auto max-h-[75vh] object-contain rounded-xl shadow-md group-hover:scale-[1.01] transition-transform duration-300 ease-out"
                                    referrerPolicy="no-referrer"
                                  />
                                  <div className="absolute inset-0 bg-slate-950/0 group-hover:bg-slate-950/15 transition-colors duration-300 rounded-2xl flex items-center justify-center">
                                    <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-slate-950/90 text-white text-xs font-black px-4 py-2 rounded-xl border border-white/20 shadow-xl tracking-wider uppercase flex items-center gap-2 transform translate-y-2 group-hover:translate-y-0">
                                      <span>🔍 View Full Size & Scroll</span>
                                    </span>
                                  </div>
                                </div>
                              )}
                              ...
                              {/* Badge icon & index */}
                              <div className="absolute bottom-3 right-3 bg-slate-950/85 backdrop-blur-md text-[10px] font-black px-2.5 py-1 rounded-lg text-white border border-white/20 flex items-center gap-1.5 shadow-md">
