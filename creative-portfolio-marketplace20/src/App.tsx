...
export default function App() {
  ...
  useEffect(() => {
    const parseUrlHash = () => {
      ...
      const creative = hashParams.get('creative') || hashParams.get('username') || searchParams.get('creative') || searchParams.get('username');
  ...
  useEffect(() => {
    if (currentTab === 'home') {
      ...
      let newHash = '#browse';
      if (sharedUsername) {
        newHash = sharedItemId ? `#creative=${sharedUsername}&item=${sharedItemId}` : `#creative=${sharedUsername}`;
      }
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `${window.location.pathname}${newHash}`);
      }
  ...
  }, [currentTab, selectedJobId, sharedUsername, sharedItemId]);
  ...
  useEffect(() => {
    ...
    if (profileUser) {
      ...
      if (found) {
        setSharedUsername(found.username);
