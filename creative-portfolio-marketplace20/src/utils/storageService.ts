...
/**
 * Synchronously returns a public or cached signed URL for a Supabase Storage path.
 */
export function getPublicOrSignedUrl(path: string | null | undefined): string {
  if (!path) return '';
  ...
  try {
    const { data } = supabase.storage.from('app-files').getPublicUrl(trimmed);
    if (data?.publicUrl) {
...
/**
 * Generates a signed or public URL for a given Supabase Storage path.
 * Uses a global cache and public URL fallback to resolve instantly.
 */
export async function getSignedUrl(path: string | null | undefined): Promise<string> {
  ...
  return getPublicOrSignedUrl(trimmed);
}
...
export async function uploadBase64(
  ...
  try {
    ...
    if (error) {
      console.warn('Supabase storage upload error (using base64 fallback):', error.message || error);
      return base64;
    }
    return filePath;
  } catch (err: any) {
    console.warn('Error uploading base64 to storage (using base64 fallback):', err?.message || err);
    return base64;
  }
}
...
export async function processProfileUploads(profile: FreelancerProfile): Promise<FreelancerProfile> {
  try {
    ...
    console.warn('Process profile uploads failed safely (using profile data):', err);
    return profile;
  }
}
...
export async function resolveProfileUrls(profile: FreelancerProfile): Promise<FreelancerProfile> {
  try {
    ...
    if (isStoragePath(updated.avatarUrl)) {
      updated.avatarUrl = getPublicOrSignedUrl(updated.avatarUrl);
    }
    if (isStoragePath(updated.originalAvatarUrl)) {
      updated.originalAvatarUrl = getPublicOrSignedUrl(updated.originalAvatarUrl);
    }

    // Cover
    if (isStoragePath(updated.coverUrl)) {
      updated.coverUrl = getPublicOrSignedUrl(updated.coverUrl);
    }
    if (isStoragePath(updated.originalCoverUrl)) {
      updated.originalCoverUrl = getPublicOrSignedUrl(updated.originalCoverUrl);
    }
    ...
    if (updated.portfolio && updated.portfolio.length > 0) {
      updated.portfolio = updated.portfolio.map((item) => {
        ...
        if (isStoragePath(itemCopy.imageUrl)) {
          itemCopy.imageUrl = getPublicOrSignedUrl(itemCopy.imageUrl);
        }
        if (isStoragePath(itemCopy.videoUrl)) {
          itemCopy.videoUrl = getPublicOrSignedUrl(itemCopy.videoUrl);
        }
        if (itemCopy.galleryUrls && itemCopy.galleryUrls.length > 0) {
          itemCopy.galleryUrls = itemCopy.galleryUrls.map((gUrl) => {
            if (isStoragePath(gUrl)) {
              return getPublicOrSignedUrl(gUrl);
            }
            return gUrl;
          });
        }
    ...
    if (updated.feedPosts && updated.feedPosts.length > 0) {
      updated.feedPosts = updated.feedPosts.map((post) => {
        ...
        if (isStoragePath(postCopy.imageUrl)) {
          postCopy.imageUrl = getPublicOrSignedUrl(postCopy.imageUrl);
        }
        return postCopy;
      });
    }
    ...
    if (updated.notableClients && updated.notableClients.length > 0) {
      updated.notableClients = updated.notableClients.map((client) => {
        ...
        if (isStoragePath(clientCopy.logoUrl)) {
          clientCopy.logoUrl = getPublicOrSignedUrl(clientCopy.logoUrl);
        }
        return clientCopy;
      });
    }
    ...
    console.warn('Resolve profile URLs failed safely (using profile data):', err);
    return profile;
  }
}
