...
import { MapPin, Star, Sparkles, ArrowRight, ChevronLeft, ChevronRight, Search, SlidersHorizontal, ChevronDown, X, DollarSign, ArrowUpDown, Share2, Check } from 'lucide-react';
...
export default function BrowseTalent({ 
  ...
  const handleShareCreative = (freelancer: FreelancerProfile) => {
    const shareUrl = `${window.location.origin}${window.location.pathname}#creative=${freelancer.username || freelancer.id}`;
  ...
  const categories: { id: CreativeCategory | 'all'; label: string; icon: string; count: number }[] = [
    ...
    { id: 'dops', label: 'Directors of Photography', icon: '📹', count: freelancers.filter(f => f.category === 'dops').length },
    ...
    { id: 'filmdirectors', label: 'Film Directors', icon: '🎬', count: freelancers.filter(f => f.category === 'filmdirectors').length },
    ...
    { id: 'framingmoulds', label: 'Framing & Photo Moulds', icon: '🖼️', count: freelancers.filter(f => f.category === 'framingmoulds').length },
    ...
    { id: 'craneoperators', label: 'Jib & Crane Operators', icon: '🏗️', count: freelancers.filter(f => f.category === 'craneoperators').length },
    { id: 'lightingdesigners', label: 'Lighting Designers', icon: '💡', count: freelancers.filter(f => f.category === 'lightingdesigners').length },
    { id: 'livestream', label: 'Livestream & Broadcast', icon: '📡', count: freelancers.filter(f => f.category === 'livestream').length },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-3 sm:p-5 space-y-3 sm:space-y-4">
        {/* Category Dropdown Selection */}
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] sm:text-xs font-semibold text-slate-450 tracking-wide">Select Creative Category</h3>
            {selectedCategory !== 'all' && (
              <button
                onClick={() => setSelectedCategory('all')}
                className="text-[9px] sm:text-[10px] font-bold text-indigo-600 hover:text-indigo-700 uppercase cursor-pointer"
              >
                Show All Categories
              </button>
            )}
          ...
          <div className="relative">
            <select
              ...
              className="w-full pl-3 pr-10 py-2 sm:py-2.5 bg-slate-50 hover:bg-slate-150/50 border border-slate-100 rounded-xl text-xs sm:text-sm font-semibold text-slate-700 transition-all cursor-pointer appearance-none outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 tracking-wide"
            >
              {categories.map((cat) => (
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-2.5">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 h-3.5 w-3.5 sm:h-4 sm:w-4" />
            <input
              type="text"
              placeholder="Search for creatives by name, title, or skills..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-8 sm:pl-9 pr-9 sm:pr-10 py-1.5 sm:py-2 bg-slate-50 rounded-xl border border-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:bg-white text-slate-900 text-xs sm:text-sm font-medium transition-all placeholder:text-slate-400"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-2.5 sm:right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-1 rounded-md hover:bg-slate-100 cursor-pointer transition-colors"
                title="Clear Search"
              >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl border transition-all font-semibold cursor-pointer text-xs sm:text-xs w-full sm:w-auto ${
                showFilters
              ...
              >
                ...
                <div className="relative h-32 md:h-36 bg-slate-100 overflow-hidden">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleShareCreative(freelancer);
                    }}
                    className="absolute top-2.5 right-2.5 p-2 bg-slate-900/80 hover:bg-indigo-600 text-white rounded-xl transition-all shadow-md backdrop-blur-xs cursor-pointer border border-white/20 z-10"
                    title="Share direct link to this creative"
                  >
                ...
                <div className="p-5 md:p-6 flex-1 flex flex-col justify-between space-y-4">
                  <div className="flex gap-4 items-start relative">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <h2 className="text-xs sm:text-base md:text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors truncate">
                          {freelancer.fullName}
                        </h2>
                        {avgRating !== null ? (
                          <div className="flex items-center gap-1 shrink-0 text-indigo-600 font-bold text-xs sm:text-sm bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100/40">
                      ...
                      <p className="text-[10px] sm:text-xs md:text-sm font-semibold text-indigo-600 truncate">{freelancer.title}</p>
                  ...
                  {/* Bio snippet */}
                  <p className="text-[9px] sm:text-xs md:text-sm text-slate-500 line-clamp-2 leading-relaxed">
                    {freelancer.bio}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {freelancer.skills.slice(0, 3).map((skill, index) => (
                      <span key={index} className="text-[8px] sm:text-[9px] md:text-xs px-2.5 py-1 bg-slate-50 text-slate-600 rounded-lg font-medium border border-slate-100">
                        {skill}
                      </span>
                    ))}
                    {freelancer.skills.length > 3 && (
                      <span className="text-[8px] sm:text-[9px] md:text-xs px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg font-bold">
                        +{freelancer.skills.length - 3}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-100/80 flex items-center justify-between">
                    <button
                      ...
                      className="p-1.5 bg-slate-100 hover:bg-indigo-50 text-slate-500 hover:text-indigo-600 rounded-lg transition-colors cursor-pointer border border-slate-200 flex items-center gap-1 text-xs font-semibold"
                      title="Share direct profile link"
                    >
                      <Share2 className="h-3.5 w-3.5" />
